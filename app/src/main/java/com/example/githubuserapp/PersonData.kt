    ...
    val listData: ArrayList<Person>
        get(){
            ...
            for (i in nameData.indices){
                val person = Person()
                person.username = usernameData[i]
                person.name = nameData[i]
                person.image = imgData[i]
                person.follower = followerData[i]
                person.following = followingData[i]
                person.repository = repositoryData[i]
                person.company = companyData[i]
                person.location = locationData[i]
                list.add(person)
            }
            return list
    }
